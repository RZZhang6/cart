...
import (
	...
	"github.com/RZZhang6/cart/domain/service"
...
//添加购物车
func (h *Cart) AddCart(ctx context.Context, request *cart.CartInfo, response *cart.ResponseAdd) error {
	cart := &model.Cart{}
	common.SwapTo(request, cart)
	addCartId, err := h.CartDataService.AddCart(cart)
	response.CartId = addCartId
	return err
}
